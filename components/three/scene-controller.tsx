...
function RingNodes({ 
  ...
  useFrame(({ clock }) => {
    ...
    nodes.forEach((node, i) => {
      ...
      const currentAngle = (i / nodes.length) * Math.PI * 2 + nodeT * 0.4 // Changed from 0.2
...
function Ring({ 
  ...
  useFrame(({ clock }) => {
    ...
    ringRef.current.rotation.x = rotation[0] + Math.sin(t * 0.4) * 0.1 // Changed from 0.2
    ringRef.current.rotation.y = rotation[1] + t * 0.3 // Changed from 0.15
    ringRef.current.rotation.z = rotation[2] + Math.cos(t * 0.6) * 0.08 // Changed from 0.3
